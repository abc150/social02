 ...
 * @desc 포인트의 양을 유지할 변수의 정의입니다.
 * @default 5
 *
 * @param Result Switch ID
 * @desc 미니 게임에서 승패에 해당하는 스위치의 정의입니다. Definição da switch que corresponde a vitória ou derrota do minigame.
 * @default 10 
 *
 * @param Score Base
 * @desc 포인트의 기본 값 정의. Definição do valor basico dos pontos.
 * @default 10
 *
 * @param Chain Bonus
 * @desc 콤보를 통해 획득한 포인트의 가치를 설정합니다. Definição do valor dos pontos ganhos fazer combos.
 * @default 5 
 *
 * @param Critical Bonus Percentage
 * @desc 크리티컬 데미지를 입힐 때의 퍼센트 설정. Definição da porcentagem ao fazer danos críticos.
 * @default 1.5  
 *
 * @param Max Chain Bonus Percentage
 * @desc 최종 결과에서 얻은 백분율의 정의입니다. Definição da porcentagem ganho no resultado final.
 * @default 0.01
 *
 * @param Miss Dmg Percentage
 * @desc 버튼이 없거나 누락된 경우 손상 비율을 설정합니다. Definição da porcentagem de dano ao errar ou deixar o botão passar.
 * @default 0
 *
 * @param BP Base Value
 * @desc 전투력의 기본 값을 설정합니다. Definição do valor base do Battle Power.
 * @default 50
 *
 * @param Enemy Atk Animation
 * @desc 적 공격의 기본 애니메이션 정의입니다. Definição da animação padrão do ataque inimigo.
 * @default 1
 *
 * @param >> POSITION ===================
 * @desc
